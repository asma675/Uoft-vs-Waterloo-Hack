...
export type DemoMissionSeed = {
  id?: string
  approvalToken?: string
  timestamp?: string
}

export function createDemoMission(goal = DEFAULT_GOAL, seed: DemoMissionSeed = {}): Mission {
  const approvalToken = seed.approvalToken ?? makeId("APR")
  ...
  return {
    ...
    approvalToken,
    events: [
      { id: "evt-1", at: timestamp, actor: "system", type: "session", message: "Mission created with three isolated research roles." },
      { id: "evt-2", at: timestamp, actor: "scout", type: "research", message: "Candidate sources and product claims collected." },
      { id: "evt-3", at: timestamp, actor: "verifier", type: "verification", message: "Core product claims independently corroborated." },
      { id: "evt-4", at: timestamp, actor: "signalshield", type: "threat", message: "AI-targeted instruction and hidden content detected on deal-drop.example." },
      { id: "evt-5", at: timestamp, actor: "adversary", type: "threat", message: "Suspicious source quarantined and excluded from consensus." },
      { id: "evt-6", at: timestamp, actor: "system", type: "score", message: `Explainable Trust Score resolved to ${score}/100.` },
    ],
    threat: {
      detected: true,
      host: "deal-drop.example",
