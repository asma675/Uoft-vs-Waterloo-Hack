import type { EvidenceItem, Mission } from "@/lib/trustmesh"
import type { SourceRecord } from "@/lib/trustmesh"
...
export async function planSources(goal: string): Promise<{ sources: PlannedSource[]; actionRisk: Mission["actionRisk"] }> {
    ...
    input: [
      {
        role: "system",
        content:
          "You are the discovery planner for TrustMesh, a zero-trust browser verification system. Find independent, directly relevant web sources for the user's goal. Prefer primary/official or reputable sources. Never authorize or execute an external action. Avoid returning duplicate domains when possible.",
      },
  ...
  return {
    sources: parsed.sources.filter((source) => /^https?:\/\//i.test(source.url)).slice(0, 4),
    actionRisk: parsed.actionRisk,
  }
}
...
export async function synthesizeEvidence(goal: string, sources: SourceRecord[]): Promise<{
  ...
  return {
    evidence: parsed.evidence.map((item, index) => ({
      ...
      sources: new Set(item.sourceIds).size,
      status: item.status,
